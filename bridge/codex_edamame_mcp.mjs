...
const CONTROL_CENTER_TOOL_NAME = "edamame_codex_control_center";
const CONTROL_CENTER_REFRESH_TOOL_NAME = "edamame_codex_control_center_refresh";
const CONTROL_CENTER_REFRESH_NOW_TOOL_NAME = "edamame_codex_control_center_refresh_now";
const CONTROL_CENTER_APPLY_PAIRING_TOOL_NAME = "edamame_codex_control_center_apply_pairing";
const CONTROL_CENTER_RUN_HOST_ACTION_TOOL_NAME = "edamame_codex_control_center_run_host_action";
const CONTROL_CENTER_REQUEST_APP_PAIRING_TOOL_NAME = "edamame_codex_control_center_request_app_pairing";
const REFRESH_TOOL_NAME = "codex_refresh_behavioral_model";
const LEGACY_REFRESH_TOOL_NAME = "codex.refresh_behavioral_model";
const HEALTHCHECK_TOOL_NAME = "codex_healthcheck";
const LEGACY_HEALTHCHECK_TOOL_NAME = "codex.healthcheck";
const POSTURE_SUMMARY_TOOL_NAME = "codex_posture_summary";
const LEGACY_POSTURE_SUMMARY_TOOL_NAME = "codex.posture_summary";

export const TOOL_DEFINITIONS = [
  {
    name: CONTROL_CENTER_TOOL_NAME,
    title: "Codex CLI EDAMAME Control Center",
    description:
  ...
  },
  {
    ...
    title: "Refresh Codex CLI EDAMAME Control Center",
  ...
  },
  {
    ...
    title: "Apply Codex CLI EDAMAME Pairing",
    description:
      "Store the local EDAMAME MCP endpoint and credential (PSK or per-client token) for the Codex CLI bridge, then return an updated control-center snapshot.",
    annotations: {
      title: "Apply Codex CLI EDAMAME Pairing",
    },
    _meta: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        psk: {
          type: "string",
          description: "EDAMAME MCP credential (PSK or per-client token) to store locally for this Codex CLI bridge.",
        },
      },
    },
  },
  {
    ...
    title: "Run Codex CLI EDAMAME Host Action",
    description:
  ...
  },
  {
    ...
    description:
      "Read recent Codex CLI transcripts, forward raw reasoning sessions to EDAMAME, and let EDAMAME's configured LLM build and upsert the BehavioralWindow.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description:
      "Check local Codex CLI package configuration, EDAMAME MCP reachability, divergence-engine status, and behavioral-model presence.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
export const RESOURCE_DEFINITIONS = [
  {
    ...
    name: "Codex CLI EDAMAME Control Center",
    description:
      "Interactive setup and status dashboard for Codex CLI-to-EDAMAME pairing, extrapolation state, and posture summary.",
...
function backgroundRefreshLoggingEnabled(config) {
  ...
  const envValue = String(process.env.CODEX_EDAMAME_LOG_REFRESH || "")
    .trim()
    .toLowerCase();
...
export function createCodexCodeDrivenRefresh(config, options = {}) {
  const runExtrapolation = options.runExtrapolation || runLatestExtrapolation;
  ...
  async function maybeRun(trigger, options = {}) {
    ...
    if (!force) {
      const persistedState = await loadStateFn(config, "codex-extrapolator", null);
    ...
    const runPromise = (async () => {
      ...
      try {
        ...
        if (logRefresh) {
          stderrLog(
            `codex_refresh trigger=${trigger} success=${result?.success !== false} upserted=${result?.upserted === true} sessions=${result?.sessionCount ?? 0} reasons=${formatRefreshReasons(result)}`,
          );
        }
        return { skipped: false, trigger, result };
      } catch (error) {
        if (logRefresh) {
          stderrLog(
            `codex_refresh trigger=${trigger} unexpected_error=${String(error?.stack || error)}`,
          );
        }
        return { skipped: false, trigger, error };
      } finally {
...
async function main() {
  ...
  const autoRefresh = createCodexCodeDrivenRefresh(config);
