<#
.SYNOPSIS
    Installs EDAMAME for Codex CLI (global per-user install) on Windows.
...
$ConfigHome = Join-Path $env:APPDATA "codex-edamame"
$StateHome  = Join-Path $env:LOCALAPPDATA "codex-edamame\state"
$DataHome   = Join-Path $env:LOCALAPPDATA "codex-edamame"

$InstallRoot = Join-Path $DataHome "current"
$ConfigPath  = Join-Path $ConfigHome "config.json"
$CodexCodeMcpPath = Join-Path $ConfigHome "codex-mcp.json"
...
$ConfigTemplate = Join-Path $InstallRoot "setup\codex-edamame-config.template.json"
if ((-not (Test-Path $ConfigPath)) -and (Test-Path $ConfigTemplate)) {
    Render-Template $ConfigTemplate $ConfigPath
}

$McpTemplate = Join-Path $InstallRoot "setup\codex-mcp.template.json"
if (Test-Path $McpTemplate) {
    Render-Template $McpTemplate $CodexCodeMcpPath
}

# --- MCP auto-injection into ~/.codex/config.toml ---
$CodexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $env:USERPROFILE ".codex" }
$CodexConfigPath = Join-Path $CodexHome "config.toml"
try {
    $SnippetContent = Get-Content -Raw $CodexCodeMcpPath | ConvertFrom-Json
    $Entry = $SnippetContent.mcpServers.'edamame-codex'
    if ($Entry) {
        ...
        if (Test-Path $CodexConfigPath) {
            Copy-Item -Force $CodexConfigPath "$CodexConfigPath.bak"
        ...
        if ($Existing -match [regex]::Escape($Marker)) {
            ...
            $Existing = ($Out -join "`n").TrimEnd() + "`n"
        }
        $ArgsJson = ($Entry.args | ConvertTo-Json -Compress)
        $Block = "[mcp_servers.edamame-codex]`ncommand = `"$($Entry.command)`"`nargs = $ArgsJson`n"
        (($Existing.TrimEnd() + "`n`n" + $Block).TrimStart()) | Set-Content -Path $CodexConfigPath -Encoding UTF8
    }
} catch {
    Write-Warning "Could not inject Codex MCP entry: $_"
}
...
MCP server registered automatically in ~\.codex\config.toml
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBIuZ2GO3ZU8Ww9
# 6zcIkxus0tDDxgM5qyYuFCFeserB1aCCE38wggVyMIIDWqADAgECAhB2U/6sdUZI
...
# NwIBFTAvBgkqhkiG9w0BCQQxIgQgN1cn12B8CMwakEEWMGk2CcqxtOypiu2rOwzb
# nf1koqIwDQYJKoZIhvcNAQEBBQAEggIALicAklxQ0wsSaccCcGgPAU+kM6+RkC1n
# exDDSgnhCyk7k0he5nDu/xW36QZnCfH8BhRbEpIB4+oRVF8fOy4LFO2lWiENPZiy
# dSX+7zaj2P1oHzy6oDV/OWWtjq61E611/syT+cwReOosbDgBAixzYUDFqPqDRnHQ
# 0jALOrJHGCto//hdlP7Zgf1WGI17wqOzjJ8pL+FQJi3atDJhCRQHcsFdCDLHDgB9
# Z022jB59mQcc7RiXPRrQlfx2vt1XcAwwnalnTXV0pmG2eiiFuSTfhLatndNfT/Ep
# gelLHbD6DBWQS8vtXw6PCW2PcW1XTA3k8fTrlAYyh8xba0k+yyfJhtTMR/qfl1Rx
