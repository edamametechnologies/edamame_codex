# Codex CLI Intent E2E Test

End-to-end test for the Codex CLI reasoning-plane pipeline: synthetic Codex CLI
transcripts are injected, processed by the `codex_extrapolator`, and
...
1. **Provision checks** -- installed package layout, version alignment between
   repo and installed copy, MCP snippet presence, PSK file, Codex CLI plugin
   registration.
2. **Synthetic transcript generation** -- three Codex CLI-style plain text
   transcript files (`e2e_api_*`, `e2e_shell_*`, `e2e_git_*`) are written under
   the Codex projects root.
3. **Extrapolator execution** -- `codex_extrapolator.mjs` processes the
...
| `CODEX_EDAMAME_CONFIG` | platform default | Override `config.json` path |
| `E2E_SKIP_PLUGIN_CHECK` | 0 | If 1, skip Codex CLI marketplace / MCP path checks |
...
under `tests/e2e/`. Run triggers with `--agent-type codex`. See
