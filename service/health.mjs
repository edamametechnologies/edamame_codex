...
export async function runHealthcheck(config, options = {}) {
  ...
  addCheck("config.codexSessionsRoot", !!config.codexSessionsRoot, config.codexSessionsRoot);
  ...
  const extrapolatorState = await loadState(config, "codex-extrapolator", null);
  if (extrapolatorState) {
    const lastError =
      typeof extrapolatorState.lastError === "string" && extrapolatorState.lastError.trim()
        ...
        : null;
    addCheck("codex.extrapolator", !lastError, {
