...
export function describeExtrapolationFailure(failure) {
  if (!failure || typeof failure !== "object") {
    return {
      ...
      summary: "The Codex CLI bridge hit an unknown EDAMAME error.",
    };
  }
  ...
  switch (failure.reason) {
    ...
    case "edamame_mcp_unreachable":
      return {
        ...
        summary:
          "The Codex CLI bridge could not reach the configured EDAMAME MCP endpoint.",
      };
    ...
    default:
      return {
        ...
        summary: "The Codex CLI bridge hit an EDAMAME error.",
      };
  }
}
...
export async function runLatestExtrapolation(config, options = {}) {
  ...
  const previousState = (await loadStateFn(config, "codex-extrapolator", {})) || {};
  ...
  if (sessions.length === 0 && !cachedGeneratedWindow) {
      ...
      predictions: [
        {
          ...
          action:
            "Periodic Codex CLI extrapolator cron tick with no new reasoning activity to model.",
          tools_called: [],
          ...
          expected_traffic: [...(config.codexLlmHosts || [])],
    ...
    try {
      ...
      await saveStateFn(config, "codex-extrapolator", {
