...
import { runLatestExtrapolation } from "./codex_extrapolator.mjs";
...
function baselineConfigJson(config) {
  return {
    ...
    codex_sessions_root: config.codexSessionsRoot,
    ...
    codex_llm_hosts: config.codexLlmHosts,
...
function summaryLine({ pairingConfigured, posture, health, hostActionResult, refreshResult, extrapolator }) {
  ...
  if (!pairingConfigured) return "Pair the local Codex CLI bridge with EDAMAME to enable status reads.";
  const authFailure =
...
export async function buildControlCenterPayload(config, options = {}) {
  ...
  const extrapolatorState = compactExtrapolatorState(
    await loadState(config, "codex-extrapolator", {}),
  );
...
export async function applyPairing(config, args = {}) {
  ...
  try {
    const extState = await loadState(config, "codex-extrapolator", {});
    if (
      ...
      await saveState(config, "codex-extrapolator", extState);
    }
  } catch (_e) {
...
const APP_PAIRING_CLIENT_NAME = "EDAMAME for Codex CLI";
...
export async function requestAppPairing(config, args = {}) {
  ...
  const body = {
    ...
    agent_type: String(config.agentType || "codex"),
