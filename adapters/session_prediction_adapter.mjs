...
function looksLikeCodexCodeProjectPath(filePath, hints) {
...
async function candidateTranscriptFiles(config) {
  const allFiles = await walkFiles(config.codexSessionsRoot);
  const recencyCutoff = Date.now() - config.transcriptRecencyHours * 60 * 60 * 1000;
  const hintCandidates = allFiles.filter((filePath) =>
    looksLikeCodexCodeProjectPath(filePath, config.transcriptProjectHints || []),
  );
...
function extractTraffic(text, commands, config) {
  ...
  for (const host of config.codexLlmHosts || []) {
    if (host.includes(":")) {
      hosts.push(host);
    } else {
      hosts.push(`${host}:443`);
...
export async function collectTranscriptSessions(config, options = {}) {
  ...
  for (const candidate of limited) {
    ...
    records.push({
      ...
      title: firstNonEmptyLine(userText) || `Codex CLI session ${candidate.sessionId}`,
...
export async function buildRawSessionIngestPayload(config, options = {}) {
  ...
  const agentType = config.agentType || "codex";
  const agentInstanceId = config.agentInstanceId || "codex-default";
  ...
  const rawSessions = {
    ...
    sessions: sessions.map((session) => {
      ...
      return {
        ...
        derived_expected_traffic: uniqueStrings([...derivedExpectedTraffic, ...(config.codexLlmHosts || [])]),
