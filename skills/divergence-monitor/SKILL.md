...
description: Monitor and diagnose behavioral divergence between Codex CLI intent and observed system activity. Use when the user asks about divergence verdicts, behavioral model status, or wants to debug why EDAMAME flagged an alert.
---
...
- User asks about the behavioral model or what EDAMAME thinks Codex CLI is doing
...
1. Start by calling `edamame_codex_control_center` to get the full status snapshot including:
...
4. If the verdict is `NO_MODEL`, guide the user to export intent:
   - Use `edamame_codex_control_center_refresh_now` to push current transcripts
...
- Multiple agent contributors (Codex CLI, Cursor, OpenClaw, etc.) may be merged into one model. The Codex CLI slice is identified by `agent_type=codex`.
