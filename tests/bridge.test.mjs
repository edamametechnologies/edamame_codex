...
import {
  ...
  createCodexCodeDrivenRefresh,
  handleRequest,
  tryExtractMessages,
} from "../bridge/codex_edamame_mcp.mjs";
import { buildControlCenterPayload } from "../service/control_center.mjs";
import { runHealthcheck } from "../service/health.mjs";
import { runLatestExtrapolation } from "../service/codex_extrapolator.mjs";
...
async function makeBridgeFixture(options = {}) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "codex-edamame-bridge-"));
    ...
    JSON.stringify(
      {
        ...
        codex_sessions_root: claudeProjectsRoot,
...
`,
  ...
  return {
    configPath,
    workspaceRoot,
    codexSessionsRoot: claudeProjectsRoot,
    ...
    agentType: "codex",
    agentInstanceId: "codex-bridge-test",
    hostKind,
...
test("handleRequest returns MCP initialize and tool list responses", async () => {
  ...
  assert.ok(
    toolsResponse.result.tools.some((tool) => tool.name === "codex_refresh_behavioral_model"),
  );
  const controlCenterTool = toolsResponse.result.tools.find(
    (tool) => tool.name === "edamame_codex_control_center",
  );
  assert.ok(controlCenterTool, "control center tool should be defined");
  const appPairingTool = toolsResponse.result.tools.find(
    (tool) => tool.name === "edamame_codex_control_center_request_app_pairing",
  );
  ...
  assert.equal(
    toolsResponse.result.tools.some((tool) => tool.name === "edamame_codex_control_center_apply_pairing"),
    true,
  );
  assert.equal(
    toolsResponse.result.tools.some((tool) => tool.name === "edamame_codex_control_center_run_host_action"),
    true,
  );
...
test("bridge can dispatch dry-run extrapolation and healthcheck tools", async () => {
  ...
  const extrapolatorResponse = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "codex_refresh_behavioral_model",
  ...
  assert.equal(extrapolatorPayload.agentType, "codex");
  assert.equal(extrapolatorPayload.agentInstanceId, "codex-bridge-test");
  assert.equal(extrapolatorPayload.rawSessions.agent_type, "codex");
  assert.equal(extrapolatorPayload.rawSessions.agent_instance_id, "codex-bridge-test");
  assert.equal(extrapolatorPayload.rawSessions.source_kind, "codex");
...
test("control center pairing stores PSK and updates config", async () => {
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame_codex_control_center_apply_pairing",
      arguments: {
...
test("control center app pairing keeps the canonical Codex CLI client name", async () => {
  ...
  try {
    const response = await handleRequest(config, {
      ...
      method: "tools/call",
      params: {
        name: "edamame_codex_control_center_request_app_pairing",
        arguments: {
          endpoint,
    ...
    assert.equal(pairingBodies[0].client_name, "Codex CLI");
    assert.equal(pairingBodies[0].agent_type, "codex");
    assert.equal(pairingBodies[0].agent_instance_id, "codex-bridge-test");
...
test("control center can auto-pair a local posture host", { skip: process.platform === "win32" }, async () => {
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame_codex_control_center_run_host_action",
      arguments: {
...
test("codex-driven refresh skips recent persisted runs", async () => {
  const refresh = createCodexCodeDrivenRefresh(
    { divergenceIntervalSecs: 120 },
    {
      loadState: async () => ({
...
test("handleRequest serves the control-center app resource", async () => {
  ...
  assert.match(resourcesResponse.result.contents[0].text, /Codex CLI EDAMAME Control Center/);
});
...
test("bridge can dispatch the control center tool with structured content", async () => {
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame_codex_control_center",
      arguments: {},
    },
  });
...
test("legacy control center aliases are rejected", async () => {
  ...
  const dottedAliasResponse = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "edamame.codex_control_center",
      arguments: {},
    },
  });

  assert.equal(dottedAliasResponse.error.message, "unsupported_tool:edamame.codex_control_center");
  ...
  const response = await handleRequest(config, {
    ...
    method: "tools/call",
    params: {
      name: "codex.control_center",
      arguments: {},
    },
  });

  assert.equal(response.error.message, "unsupported_tool:codex.control_center");
});

test("handleRequest uses Codex CLI lifecycle refresh hooks", async () => {
  const config = await makeBridgeFixture();
  const calls = [];
  const autoRefresh = {
  ...
  assert.deepEqual(calls, [
    ...
    { type: "maybeRun", trigger: "tool_call:codex_healthcheck" },
  ]);
});
...
test("codex-driven refresh reruns when transcript changed after recent persisted run", async () => {
  const now = Date.now();
...
test("runLatestExtrapolation skips unchanged payload when matching contributor is already remote", async () => {
  ...
  const result = await runLatestExtrapolation(config, {
    buildPayload: async () => ({
      ...
      rawSessions: {
        agent_type: "codex",
        agent_instance_id: "codex-bridge-test",
        source_kind: "codex",
    ...
    loadState: async (_cfg, name) => {
      assert.equal(name, "codex-extrapolator");
      return {
...
test("runLatestExtrapolation pushes heartbeat window when no sessions and no cached window", async () => {
  ...
  assert.equal(heartbeat.agent_type, "codex");
  assert.equal(heartbeat.agent_instance_id, "codex-bridge-test");
  ...
  assert.equal(savedStates[0].lastGeneratedWindow.agent_type, "codex");
});
